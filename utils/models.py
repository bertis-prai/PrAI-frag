...
class Multiply(nn.Module):
  ...
  def forward(self, tensors):
    ...
    return t
#
# class PeptideIRTNet(nn.Module):
#     def __init__(self, config):
#         super(PeptideIRTNet, self).__init__()
#
...
#         self.featlinear = nn.Linear(2, hidden_size *2)
...
#         h_gru_atten = self.gru_attention(h_gru2) # ## 32 x 15 X 512
...
#         hgru3, _h3 = self.gru3(repeat_vector_output, _h2)
...
#         self.linear = nn.Linear(hidden_size * 2, hidden_size)
#         self.linear2 = nn.Linear(hidden_size * 2, hidden_size *2)
#         self.featlinear = nn.Linear(3, hidden_size *2)
#         self.multiply = Multiply()
#
#         #self.mult_lineared = nn.Linear(hidden_size * hidden_size, hidden_size)
#         self.linear_relu = nn.ReLU()
...
#
#
class PeptideIRTNet2(nn.Module):
    def __init__(self, config):
        super(PeptideIRTNet2, self).__init__()
        ...
        self.linear = nn.Linear(hidden_size * 2, hidden_size)
        self.linear2 = nn.Linear(hidden_size * 2, hidden_size *2)
        self.featlinear = nn.Linear(4, hidden_size *2)
        self.multiply = Multiply()
...
class PeptideIRTNet3(nn.Module):
    def __init__(self, config):
        super(PeptideIRTNet3, self).__init__()
...
class PeptideIRTNet4(nn.Module):
    def __init__(self, config):
        super(PeptideIRTNet4, self).__init__()
    ...
    def forward(self, x, x_feat, x_feat2):
       ...
       #print("x_feat_after",x_feat.shape)
        ...
        feature_extracted = self.linear3(feature_extracted)
        prediction = self.prediction(feature_extracted) ## fcn out ## 80 x 16 x 1 -> last is 1
        prediction = self.tanh(5*torch.pow(prediction, 3))
        return prediction
...
class PeptideIRTNet5(nn.Module):
    def __init__(self, config):
        super(PeptideIRTNet5, self).__init__()
